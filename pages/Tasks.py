...
def load_athlete_data():
    ...
    # REMOVIDO: st.info("Carregando dados dos atletas via URL CSV pública...", icon="⏳")
    try:
        df = pd.read_csv(url)
        if df.empty: return pd.DataFrame() # Retorna DF vazio se o CSV estiver vazio

        if "ROLE" not in df.columns or "INACTIVE" not in df.columns:
            st.error(f"Colunas 'ROLE' ou 'INACTIVE' não encontradas no CSV. Verifique URL/planilha.", icon="🚨")
            return pd.DataFrame()
        ...
        for col_to_check in ["IMAGE", "PASSPORT IMAGE", "MOBILE"]:
            ...
            else: df[col_to_check] = df[col_to_check].fillna("")
        ...
        st.error("Erro: URL CSV dos atletas retornou dados vazios.", icon="🚨"); return pd.DataFrame()
    except Exception as e:
        st.error(f"Erro ao carregar/processar dados dos atletas da URL CSV: {e}", icon="🚨"); return pd.DataFrame()
...
def get_valid_user_info(user_ps_id_input: str, sheet_name: str = MAIN_SHEET_NAME, users_tab_name: str = USERS_TAB_NAME):
    ...
    for user_record in all_users:
        ...
        if ps_id_from_sheet == validation_id_from_input or \
           ...
           ps_id_from_sheet == processed_user_input: # Adicionado para caso input seja só o número
            return user_record
    return None
...
with st.container(border=True):
        ...
        if st.button("Confirmar Usuário", key="confirm_user_button", use_container_width=True, type="primary"):
            ...
            if user_input_val:
                ...
                if user_info_val:
                    ...
                    st.session_state.update({"user_confirmed": False, "current_user_image_url": "", "warning_message": f"⚠️ Usuário '{user_input_val}' não encontrado."})
            else:
                st.session_state.update({"warning_message": "⚠️ O ID/Nome do usuário não pode ser vazio.", "user_confirmed": False, "current_user_image_url": ""})
    ...
    if st.session_state.user_confirmed and st.session_state.current_user_name != "Usuário":
        ...
        if user_image_disp_url:
            st.markdown(f"""<div style="display:flex;align-items:center;gap:10px;margin-top:10px;"><img src="{html.escape(user_image_disp_url, quote=True)}" style="width:40px;height:40px;border-radius:50%;object-fit:cover;border:1px solid #555;"><div style="line-height:1.2;"><span style="font-weight:bold;">{user_name_disp}</span><br><span style="font-size:0.9em;color:#ccc;">PS: {user_id_disp}</span></div></div>""", unsafe_allow_html=True)
...
if st.session_state.user_confirmed and st.session_state.current_user_name != "Usuário":
    ...
    # Spinner para carregar config data, se necessário
    # with st.spinner("Carregando configurações..."):
    TASK_LIST_DATA, TASK_STATUS_LIST_DATA = load_config_data()

    if not TASK_LIST_DATA: st.error("Lista de tarefas não carregada da 'Config'.", icon="🚨"); st.stop()
    ...
    with ctrl_col1: 
        st.session_state.selected_task = st.selectbox("Tipo de verificação:", TASK_LIST_DATA, index=TASK_LIST_DATA.index(st.session_state.selected_task) if st.session_state.selected_task in TASK_LIST_DATA else 0, key="task_selector_widget")
    with ctrl_col2: 
        st.session_state.selected_statuses = st.multiselect("Filtrar Status:", TASK_STATUS_LIST_DATA, default=st.session_state.selected_statuses if st.session_state.selected_statuses else [], key="status_multiselect_widget")
    with ctrl_col3: 
        st.markdown("<br>", unsafe_allow_html=True)
    ...
    with st.spinner("Carregando dados dos atletas..."): # MODIFICADO AQUI
        df_athletes_loaded = load_athlete_data()      
    
    with st.spinner("Carregando registros de atividades..."): # MODIFICADO AQUI
        df_attendance_loaded = load_attendance_data()

    if df_athletes_loaded.empty:
        st.info("Nenhum dado de atleta para exibir.") 
    else:
        ...
        if task_to_filter_val and statuses_to_filter_val:
            ...
            for _, athlete_row_filter in filtered_athletes_df.iterrows():
                ...
                elif any(s in statuses_to_filter_val for s in ["Pendente", "---", "Não Registrado"]): athletes_to_show_ids_set.add(athlete_id_filter)
        ...
        for i_loop, row_data in filtered_athletes_df.iterrows():
            ...
            if "Athlete ID" in df_attendance_loaded.columns and "Task" in df_attendance_loaded.columns:
                ...
                athlete_task_records_df = df_att_task_check_disp[(df_att_task_check_disp.get("Athlete ID") == athlete_id_display) & (df_att_task_check_disp.get("Task") == st.session_state.selected_task)]
            if not athlete_task_records_df.empty and "Status" in athlete_task_records_df.columns:
                latest_record_data = athlete_task_records_df.iloc[-1] # Default to last if timestamp fails
                try:
                    if "Timestamp" in athlete_task_records_df.columns:
                        athlete_task_records_df.loc[:, "Timestamp"] = pd.to_datetime(athlete_task_records_df["Timestamp"], format="%d/%m/%Y %H:%M:%S", errors='coerce')
                        latest_record_data = athlete_task_records_df.sort_values(by="Timestamp", ascending=False).iloc[0]
                except Exception: pass # Silently ignore if sorting by timestamp fails, use default
            ...
            if current_selected_task_val == "Walkout Music":
                ...
                button_label_task, button_type_task = (f"'{current_selected_task_val}' JÁ CONCLUÍDO (Refazer?)", "secondary") if is_task_already_done else (f"Marcar '{current_selected_task_val}' como CONCLUÍDO", "primary")
                if st.button(button_label_task, key=f"mark_done_button_{athlete_id_display}_{current_selected_task_val.replace(' ', '_')}_{i_loop}", type=button_type_task, use_container_width=True):
                    user_identifier_for_log = st.session_state.get("current_user_ps_id_internal", st.session_state.current_user_id)
                    registrar_log(athlete_id_display, athlete_name_display, athlete_event_display, current_selected_task_val, "Done", "", user_identifier_for_log); st.rerun() 
