...
def registrar_log(ath_id: str, ath_name: str, ath_event: str, task: str, status: str, notes: str, user_log_id: str,
    ...
    try:
        ...
        # --- CORREÇÃO PRINCIPAL: Limpar todos os caches de dados relevantes ---
        # Isso força o Streamlit a buscar os dados mais recentes do Google Sheets na próxima execução.
...
if st.session_state.user_confirmed and st.session_state.current_user_name!="User":
    ...
    if sel_task_actual and not df_athletes.empty:
        ...
        chart_data = pd.DataFrame([
            {"Status": "Done", "Count": done_count, "Color": "#1E8449"},
            {"Status": "Requested", "Count": requested_count, "Color": "#F39C12"},
            {"Status": "Pending", "Count": pending_count, "Color": "#C0392B"}
        ])
        chart = alt.Chart(chart_data).mark_bar().encode(
            x=alt.X('Status:N', sort=None, title=None, axis=alt.Axis(labelAngle=0)),
            y=alt.Y('Count:Q', title="Nº de Atletas"),
            color=alt.Color('Status:N', scale=alt.Scale(domain=chart_data['Status'].tolist(), range=chart_data['Color'].tolist()), legend=None)
        ).properties(height=200)
        st.altair_chart(chart, use_container_width=True)
        st.divider()

    status_options = ["Todos", "Requested", "Done", "Pending"]
    st.session_state.selected_status = st.radio("Filtrar por Status:", options=status_options, index=status_options.index(st.session_state.selected_status) if st.session_state.selected_status in status_options else 1, horizontal=True, key="srad_w", disabled=(not sel_task_actual))
        ...
        for i_l, row in df_filtered.iterrows():
            ...
            with col_card:
                ...
                if mob_r:
                    ...
                    if phone_digits:
                        wa_link_html = f"""<p style='margin-top: 8px; font-size:14px; text-align:left;'><a href='https://wa.me/{html.escape(phone_digits, True)}' target='_blank' style='color:#25D366; text-decoration:none; font-weight:bold;'> WhatsApp</a></p>"""
                ...
                status_bar_color = "#C0392B" # Vermelho
                ...
                <div style='background-color:#2E2E2E; border-left: 5px solid {status_bar_color}; padding: 20px; border-radius: 10px;'>
                    <div style='display:flex; align-items:center; gap:20px;'>
                        ...
                        <div>
                            <h4 style='margin:0; font-size:1.6em; line-height: 1.2;'>{html.escape(ath_name_d)} <span style='font-size:0.6em; color:#cccccc; font-weight:normal; margin-left: 8px;'>{html.escape(ath_event_d)} (ID: {html.escape(ath_id_d)})</span></h4>{wa_link_html}
                        </div>
                    </div>{pd_tbl_h}
                </div>""", unsafe_allow_html=True)

                badges_html = "<div style='display: flex; flex-wrap: wrap; gap: 8px; margin-top: 15px;'>"
                # --- CORREÇÃO: Lógica de cores dos badges ---
                status_color_map = {"Done": "#1E8449", "Requested": "#F39C12", "---": "#34495E", "Pending": "#C0392B"}
                for task_name in tasks_raw:
                    status_for_badge = get_latest_status(ath_id_d, task_name, df_attendance)
                    color = status_color_map.get(status_for_badge, status_color_map["Pending"]) # Usa vermelho como padrão
