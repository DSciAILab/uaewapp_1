...
# CORREÇÃO: Readicionando a função que estava faltando.
def connect_gsheet_tab(gspread_client, sheet_name: str, tab_name: str):
    """Opens a specific worksheet (tab) in a spreadsheet."""
        ...
        for corner in ["Azul", "Vermelho"]:
            ...
            if col_name in df_processed.columns:
                ...
                for status, count in counts.items():
                    # Normalize different status texts to the main categories
                    if status in ["Done"]: summary[task]["Done"] += count
                    elif status in ["Requested"]: summary[task]["Requested"] += count
                    else: summary[task]["Pending"] += count # All others are pending
    return summary
...
st.markdown("""<style> ... </style>""", unsafe_allow_html=True) # CSS Omitido, permanece o mesmo

# --- Main Page Content ---
st.markdown("<h1 style='text-align: center;'>FIGHTER & TASK DASHBOARD</h1>", unsafe_allow_html=True)
st_autorefresh(interval=60000, key="dash_auto_refresh_v8")
...
if dash_data_list:
    ...
    for task, data in task_summary.items():
        chart_data = {status: count for status, count in data.items() if count > 0}
        if chart_data:
            df_chart = pd.DataFrame.from_dict(chart_data, orient='index', columns=['Count']).rename_axis('Status').reset_index()
            # Garante a ordem e as cores corretas no gráfico
            df_chart['Color'] = df_chart['Status'].map(chart_colors)
            with cols[col_index]:
                with st.container(border=True):
                    st.markdown(f"**{task}**")
                    st.bar_chart(df_chart, x='Status', y='Count', color='Color', height=200, use_container_width=True)
