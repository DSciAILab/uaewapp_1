...
/* New style for the fight order number */
.fight-order {
    font-size: 0.8em;
    color: #A0A0A0;
    font-weight: bold;
}
/* New style for custom bordered cards */
.custom-card {
    border-radius: 0.5rem;
    padding: 1rem;
    margin-bottom: 1rem;
    background-color: #262730; /* Streamlit's dark theme card color */
    border-left: 5px solid; /* Default left border style */
}
</style>
...
if task_name:
    ...
    # --- [MODIFIED] --- All rendering loops now use the dynamic border color and show fight order.
    with col1:
        ...
        for _, row in waiting_df.iterrows():
            corner_color = "blue" if row.get("Corner") == "blue" else "#e74c3c" if row.get("Corner") == "red" else "#4a4a4a"
            st.markdown(f'<div class="custom-card" style="border-left-color: {corner_color};">', unsafe_allow_html=True)
            with st.container():
                pic_col, name_col = st.columns([1, 2])
                with pic_col:
                    st.markdown(f'<img src="{row["Picture"]}" class="athlete-photo-circle">', unsafe_allow_html=True)
                with name_col:
                    fight_order = int(row['FightOrder']) if pd.notna(row['FightOrder']) else 'N/A'
                    st.write(f"**{row['Fighter']}** <span class='fight-order'>(Fight #{fight_order})</span>", unsafe_allow_html=True)
