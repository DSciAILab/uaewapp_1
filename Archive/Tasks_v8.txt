...
if st.session_state.user_confirmed and st.session_state.current_user_name!="User":
    ...
    if sel_task_actual and not df_athletes.empty:
        ...
        total_count = pending_count + requested_count + done_count
        
        st.markdown("##### Estatísticas da Tarefa")
        stat_cols = st.columns(4)
        stat_cols[0].metric("Total", f"{total_count}")
        stat_cols[1].metric("Pendente", f"{pending_count}")
        stat_cols[2].metric("Solicitado (Requested)", f"{requested_count}")
        stat_cols[3].metric("Concluído (Done)", f"{done_count}")
        st.divider()

    # --- ATUALIZAÇÃO: Botão "Todos" adicionado ---
    status_options = ["Todos", "Requested", "Done", "Pending"]
    st.session_state.selected_status = st.radio(
        "Filtrar por Status:",
        options=status_options,
        index=status_options.index(st.session_state.selected_status) if st.session_state.selected_status in status_options else 1, # Padrão para "Requested"
        horizontal=True,
        key="srad_w",
    ...
    if df_athletes.empty:
        ...
        for i_l, row in df_filtered.iterrows():
            ...
            with col_card:
                ...
                status_bar_color = "#C0392B" # Vermelho para Padrão/Pendente
                ...
                elif curr_ath_task_stat_btn == "---": status_bar_color = "#34495e" # Cinza Escuro
                ...
                # --- ATUALIZAÇÃO: Badges de status retornaram ---
                badges_html = "<div style='display: flex; flex-wrap: wrap; gap: 8px; margin-top: 15px;'>"
                status_color_map = {
                    "Done": "#1E8449", "Requested": "#F39C12", "---": "#34495E",
                }
