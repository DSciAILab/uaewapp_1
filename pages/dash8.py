...
def generate_mirrored_html_dashboard(df_processed, task_list, font_size_px):
    # (Unchanged except for the new font_size_px parameter)
    ...
    for _, row in df_processed.iterrows():
        body_html += f"<tr style='font-size: {font_size_px}px;'>" # Inject font size here
...
# The CSS no longer needs a fixed font-size for td, as it will be applied inline
...
st_autorefresh(interval=60000, key="dash_auto_refresh_v11")
...
if df_fc is None or df_fc.empty:
    st.warning("Could not load Fightcard data. Please check the spreadsheet."); st.stop()
...
# --- ATUALIZAÇÃO: Adiciona o slider de fonte na sidebar ---
st.session_state.table_font_size = st.sidebar.slider(
    "Table Font Size (px)",
    min_value=10,
    max_value=22,
    value=st.session_state.table_font_size,
    step=1
)
st.sidebar.markdown("---")
...
if dash_data_list:
    ...
    # ATUALIZAÇÃO: Passa o tamanho da fonte para a função que gera o HTML
    html_table = generate_mirrored_html_dashboard(df_dash_processed, all_tsks, st.session_state.table_font_size)
