...
# Adicionei "---" aqui para que seja tratado como um estado pendente na lógica de botões.
...
if st.session_state.user_confirmed and st.session_state.current_user_name!="User":
    ...
    if sel_task_actual:
        ...
        pending_count = sum(status_counts.get(s, 0) for s in STATUS_PENDING_EQUIVALENTS) # Simplificado
    ...
    for i_l, row in df_filtered.iterrows():
        ...
        # --- LÓGICA CONDICIONAL PARA STATUS E CORES ---
        curr_ath_task_stat = None
        status_bar_color = "#2E2E2E" # Cor neutra/invisível
        status_text_html = ""
        ...
        with col_card:
            ...
            <div style='background-color:#2E2E2E; border-left: 5px solid {status_bar_color}; padding: 20px; border-radius: 10px;'>
                ...
                {"" if not st.session_state.show_personal_data else pd_content}
        ...
        # --- [INÍCIO DA ALTERAÇÃO] Lógica de botões atualizada ---
        with col_buttons:
            if sel_task_actual:
                ...
                # Caso 2 (NOVO): A tarefa está pendente, sem registro, ou não se aplica ainda
                elif curr_ath_task_stat in STATUS_PENDING_EQUIVALENTS:
                    ...
                    if st.button("NÃO SE APLICA", key=f"not_req_b_{ath_id_d}_{i_l}", use_container_width=True):
                        # Registra o status "---" conforme solicitado
                        if registrar_log(ath_id_d, ath_name_d, ath_event_d, sel_task_actual, "---", "", uid_l): st.rerun()
                
                # Caso 3: A tarefa já foi concluída, mas pode ser solicitada novamente
