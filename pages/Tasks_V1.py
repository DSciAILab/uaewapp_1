...
st.set_page_config(page_title="UAEW | Controle de Tarefas", layout="wide")
...
with st.container(border=True):
    ...
    with col_input_ps:
        st.session_state['user_id_input'] = st.text_input("Número do PS", value=st.session_state['user_id_input'], key="uid_w", label_visibility="collapsed", placeholder="Digite os 4 dígitos do seu PS")
    ...
    with col_user_status_display:
        if st.session_state.user_confirmed and st.session_state.current_user_name != "Usuário":
            ...
            st.markdown(f'<div style="display:flex;align-items:center;height:50px;margin-top:0px;">{image_html}<div><span style="font-weight:bold;">{un}</span><br><span style="font-size:0.9em;color:#ccc;">PS: {ui}</span></div></div>', unsafe_allow_html=True)
...
if st.session_state.user_confirmed and st.session_state.current_user_name!="Usuário":
    ...
    if df_athletes.empty:
        ...
        for i_l,row in df_filtered.iterrows(): 
            ...
            wa_h = f"<tr><td style='padding-right:10px;white-space:nowrap;'><b>WhatsApp:</b></td><td><a href='https://wa.me/{html.escape(mob_r.lstrip('+'))}' target='_blank' style='color:#00BFFF;'>Msg</a></td></tr>" if mob_r else ""
            
            # CORREÇÃO: Lógica do Blood Test movida para ser usada dentro da tabela
            ...
            bt_html = f"<tr style='color:{bt_color};'><td style='padding-right:10px;white-space:nowrap;'><b>Blood Test:</b></td><td>{bt_text}{bt_expiry_text}</td></tr>"
            ...
            st.markdown(f"""<div style='background-color:{card_bg_col};padding:20px;border-radius:10px;margin-bottom:15px;box-shadow:2px 2px 5px rgba(0,0,0,0.3);'><div style='display:flex;flex-wrap:wrap;align-items:center;justify-content:space-between;gap:20px;'>
                <div style='display:flex;align-items:center;gap:15px;flex-basis:300px;flex-grow:1;'>
                    <img src='{html.escape(row.get("IMAGE",""))}' style='width:80px;height:80px;border-radius:50%;object-fit:cover;border:2px solid white;'>
                    <div>
                        ...
                        <p style='margin:0;font-size:13px;color:#a0f0a0;text-align:center;'><i>{task_stat_disp}</i></p>
                    </div>
                </div>
                {pd_tbl_h}
            </div></div>""", unsafe_allow_html=True)

            # CORREÇÃO: Restaurando a lógica de botões com a distinção para "Walkout Music"
            if sel_task_actual:
                if sel_task_actual == "Walkout Music":
                    ...
                    for j, mk_k in enumerate(m_keys):
                        st.session_state[mk_k] = st.text_input(f"Música {j+1}", value=st.session_state[mk_k], key=f"m{j+1}_in_{ath_id_d}_{i_l}", placeholder="Link YouTube")
                    ...
                    if st.button(f"Registrar Músicas para {ath_name_d}", key=f"reg_m_b_{ath_id_d}_{i_l}", type="primary", use_container_width=True):
                        ...
                        for idx, link_u in enumerate([st.session_state[k_] for k_ in m_keys]):
                            if link_u and link_u.strip():
                                ...
                                if registrar_log(ath_id_d, ath_name_d, ath_event_d, "Walkout Music", "Done", link_u.strip(), uid_l):
                                    any_m_reg = True
                                    st.session_state[m_keys[idx]] = "" # Limpa o campo após registrar
                        if any_m_reg:
                            st.rerun()
                        else:
                            st.warning("Nenhum link de música válido.", icon="⚠️")
                else:
                    # Lógica de botões para todas as outras tarefas
                    ...
                    if curr_ath_task_stat_btn == "Requested":
                        ...
                        if curr_ath_task_stat_btn == "Done":
                            btn_lbl_task = f"'{sel_task_actual}' FEITO. Solicitar Novamente?"
                            btn_type_task = "secondary"
