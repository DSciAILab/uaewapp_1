        ...
        for corner in ["Azul", "Vermelho"]:
            ...
            if col_name in df_processed.columns:
                ...
                for status, count in counts.items():
                    ...
                    else: summary[task]["Pending"] += count
    return summary
...
st_autorefresh(interval=60000, key="dash_auto_refresh_v9")
...
if dash_data_list:
    ...
    # --- UPDATED: Summary Section with st.metric ---
    ...
    for task, data in task_summary.items():
        with cols[col_index]:
            with st.container(border=True):
                st.markdown(f"<h3 style='text-align: center; margin-bottom: 10px;'>{task}</h3>", unsafe_allow_html=True)
                m_cols = st.columns(3)
