...
def load_athlete_data(sheet_name: str = MAIN_SHEET_NAME, athletes_tab_name: str = ATHLETES_TAB_NAME):
    try:
        ...
        date_cols = ["DOB", "PASSPORT EXPIRE DATE"] # Blood Test será tratado separadamente
...
def load_attendance_data(sheet_name: str = MAIN_SHEET_NAME, attendance_tab_name: str = ATTENDANCE_TAB_NAME):
    try:
        ...
        # Garante que colunas chave são do tipo string para comparações
...
def registrar_log(ath_id, ath_name, ath_event, task, status, notes, user_log_id):
    try:
        ...
        load_attendance_data.clear() # Força o recarregamento dos dados na próxima execução
        return True
    except Exception as e:
...
if st.session_state.user_confirmed and st.session_state.current_user_name!="Usuário":
    ...
    if df_athletes.empty:
        ...
        # --- ATUALIZAÇÃO: Exibição dos cards de atletas ---
        for i_l, row in df_filtered.iterrows():
            with st.container(border=True):
                ath_id_d,ath_name_d,ath_event_d=str(row.get("ID","")),str(row.get("NAME","")),str(row.get("EVENT",""))
                
                # Parte superior do card (Foto, Nome, ID, Dados Pessoais)
                col1, col2 = st.columns([0.4, 0.6])
                with col1:
                    st.image(row.get("IMAGE", "https://via.placeholder.com/150?text=No+Image"), width=150)
                ...
                with col2:
                    if st.session_state.show_personal_data:
                        ...
                        personal_data = {
                            "Gênero": row.get("GENDER", "N/A"),
                            "Nascimento": row.get("DOB", "N/A"),
                            "Nacionalidade": row.get("NATIONALITY", "N/A"),
                            "Passaporte": row.get("PASSPORT", "N/A"),
                            "Expira em": row.get("PASSPORT EXPIRE DATE", "N/A"),
                            "WhatsApp": wa_link,
                            "Passaporte Img.": pass_img_link,
                        }
                        st.table(pd.DataFrame(personal_data.items(), columns=["Campo", "Valor"]).set_index("Campo"))

                st.markdown("---") # Divisor interno do card

                # --- Grade de Status de Todas as Tarefas ---
                st.markdown("**Status de Todas as Tarefas:**")
                
                # Prepara os dados de status para este atleta
                athlete_statuses = {}
                    ...
                    for task in tasks_raw:
                        ...
                        if not task_records.empty:
                            if "Timestamp" in task_records.columns:
                                ...
                                latest = task_records.sort_values(by="TS_dt", ascending=False).iloc[0]
                            else:
                                latest = task_records.iloc[-1]
                ...
                for task, status in athlete_statuses.items():
                    bg_color = status_color_map.get(status, "#dc3545") # Vermelho como padrão
                    with status_cols[col_idx]:
                        st.markdown(f"""
                        <div style='background-color:{bg_color}; padding: 8px; border-radius: 5px; text-align:center; color:white; margin-bottom: 5px;'>
                            <small>{task}</small><br>
                            <b>{status}</b>
                        </div>
                ...
                # --- Seção de Ações para a tarefa selecionada no filtro ---
                if sel_task_actual:
                    st.markdown("---")
    ...
    if not st.session_state.user_confirmed and not st.session_state.get('warning_message'):
        st.warning("🚨 Confirme seu ID/Nome de usuário para continuar.", icon="🚨")# pages/1_Controle_de_Tarefas.py
