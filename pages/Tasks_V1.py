...
# --- Constants ---
# CORREÇÃO: Traduzindo constantes e labels de volta para o português
...
# ATUALIZAÇÃO: Adicionando 'search_query' ao estado da sessão
default_ss = {
...
with st.container(border=True):
    ...
    with col_input_ps:
        # CORREÇÃO: Ajustando placeholders para o português
        st.session_state['user_id_input'] = st.text_input(
            "Número do PS", value=st.session_state['user_id_input'], 
...
if st.session_state.user_confirmed and st.session_state.current_user_name!="Usuário":
    ...
    with cc1:
        st.selectbox("Filtrar por Tarefa:", tasks_for_select, index=tasks_for_select.index(st.session_state.selected_task) if st.session_state.selected_task in tasks_for_select else 0, key="selected_task")
    with cc2:
        st.multiselect("Filtrar por Status:", statuses_list_cfg, key="selected_statuses", disabled=(st.session_state.selected_task == NO_TASK_SELECTED_LABEL))
    with cc3:
        st.markdown("<br>", True)
        st.button("🔄 Atualizar", key="ref_b_w", help="Recarrega dados.", on_click=lambda:(load_athlete_data.clear(), load_users_data.clear(), load_config_data.clear(), load_attendance_data.clear(), st.toast("Dados atualizados!",icon="🔄"), st.rerun()), use_container_width=True)
    
    # ATUALIZAÇÃO: Campo de busca por atleta
    ...
    if df_athletes.empty:
        ...
        # 2. ATUALIZAÇÃO: Filtro por Busca de Texto
        if st.session_state.search_query:
            query = st.session_state.search_query.lower()
            df_filtered = df_filtered[
                df_filtered['NAME'].str.lower().str.contains(query) |
                df_filtered['ID'].astype(str).str.lower().str.contains(query)
            ]

        st.markdown(f"Exibindo **{len(df_filtered)}** de **{len(df_athletes)}** atletas.")
        if not sel_task_actual:
            st.info("Selecione uma tarefa para opções de registro e filtro.", icon="ℹ️")
        ...
        for i_l,row in df_filtered.iterrows(): 
            ...
            # CORREÇÃO: Simplificando a lógica do WhatsApp
            mob_r = str(row.get("MOBILE","")).strip().replace(" ","").replace("-","").replace("(","").replace(")","")
            wa_h = ""
            if mob_r:
                mob_p = mob_r if mob_r.startswith('+') else f"+{mob_r}"
                wa_h=f"<tr><td style='padding-right:10px;white-space:nowrap;'><b>WhatsApp:</b></td><td><a href='https://wa.me/{html.escape(mob_p.replace('+',''),True)}' target='_blank' style='color:#00BFFF;'>Msg</a></td></tr>"
            
            # CORREÇÃO: Removendo o Blood Test da tabela principal
            ...
            # CORREÇÃO: Renderiza o Blood Test separadamente abaixo do card
            if st.session_state.show_personal_data:
                bt_d_h,bt_ex_h=str(row.get("BLOOD TEST","")),is_blood_test_expired(str(row.get("BLOOD TEST","")))
                bt_color = "red" if bt_ex_h else ("#A0F0A0" if bt_d_h else "orange")
                bt_text = html.escape(bt_d_h) if bt_d_h else 'Não Registrado'
                bt_expiry_text = '<span style="font-weight:bold;"> (Expirado)</span>' if bt_ex_h and bt_d_h else ''
                bt_html = f"<div style='margin-top:-10px; margin-bottom:15px; font-size:14px; color:{bt_color};'><b>Blood Test:</b> {bt_text}{bt_expiry_text}</div>"
                st.markdown(bt_html, unsafe_allow_html=True)

            # Lógica dos botões de ação (inalterada)
            if sel_task_actual:
                # ... (código dos botões de ação idêntico ao anterior)
                ...
                if curr_ath_task_stat_btn=="Requested":
                    ...
                    with col_btn_act2:
                        if st.button(f"CANCELAR SOL. '{sel_task_actual}' (---)",key=f"mark_pending_b_{ath_id_d}_{sel_task_actual.replace(' ','_')}_{i_l}",type="secondary",use_container_width=True):
                            uid_l=st.session_state.get("current_user_ps_id_internal",st.session_state.current_user_id);registrar_log(ath_id_d,ath_name_d,ath_event_d,"---","",uid_l);st.rerun()
                else:
