...
if st.session_state.user_confirmed and st.session_state.current_user_id.strip().upper()!=st.session_state.user_id_input.strip().upper() and st.session_state.user_id_input.strip()!="":
    st.session_state.update(user_confirmed=False,warning_message="⚠️ ID/Nome alterado. Confirme.",current_user_image_url="",selected_task="Medical");st.rerun() # Atualizado para "Medical"
...
if st.session_state.user_confirmed and st.session_state.current_user_name!="User":
    ...
    st.session_state.selected_status = st.radio(
        f"Filtrar Atletas por Status da Tarefa '{sel_task_actual}':", # Mensagem atualizada
        ...
        horizontal=True, 
        key="srad_w"
        # 'disabled' não é mais necessário aqui, pois a tarefa está sempre selecionada
    )

    # --- Sidebar Section ---
    with st.sidebar:
        st.header("Filtros e Configurações")
        st.subheader(f"Tarefa Ativa: {sel_task_actual}") # Exibe a tarefa ativa
        ...
        st.multiselect(
            ...
            options=tasks_raw, # As opções são as tarefas em si (incluindo "Medical")
    ...
    # Não precisamos mais desta info, pois a tarefa está sempre selecionada
