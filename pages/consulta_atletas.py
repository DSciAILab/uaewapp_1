...
for i, row in df.iterrows():
    ...
    # Proteção contra erro na célula BLOOD TEST
    blood_info_html = ""
    if tipo == "Blood Test":
        blood_test_date = row.get("BLOOD TEST", "")
        if pd.notna(blood_test_date) and str(blood_test_date).strip() != "":
            blood_info_html = f"<p style='margin:0; font-size:13px;'>Blood Test in: {blood_test_date}</p>"

    button_text = "Subscrever attendance por uma nova?" if tipo == "Blood Test" and pd.notna(row["BLOOD TEST"]) and str(row["BLOOD TEST"]).strip() != "" else "Registrar Attendance"
