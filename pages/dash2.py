...
STATUS_TO_EMOJI = {
    "Done": "🟩",       # Verde
    "Requested": "🟧", # Amarelo/Laranja
    "---": "➖",        # Vermelho/Sinal de menos
    "Não Solicitado": "➖",
    "Pendente": "",      # Vazio para pendente (ou "⚪" ou "⚫")
    "Não Registrado": "" # Vazio para pendente
}
...
def load_attendance_data(sheet_name=MAIN_SHEET_NAME, attendance_tab_name=ATTENDANCE_TAB_NAME):
    ...
    try:
        ...
        return df_att # Retorna o df_att com as colunas limpas
...
# --- Nova Função para obter a representação do status da tarefa (emoji) ---
def get_task_status_representation(athlete_id_to_check, task_name, df_attendance):
    ...
    # Garante que as colunas necessárias existem no DataFrame de attendance
    ...
    # Pega o status mais recente baseado no Timestamp, se disponível
    ...
    if ATTENDANCE_TIMESTAMP_COL in relevant_records.columns:
        try:
            ...
            pass # Usa o fallback se a ordenação falhar
...
st.markdown(f"""<style> div[data-testid="stDataFrameResizable"] div[data-baseweb="table-cell"], div[data-testid="stDataFrameResizable"] div[data-baseweb="table-header-cell"] {{ font-size: {current_font_css} !important; }} div[data-testid="stDataFrameResizable"] div[data-baseweb="table-header-cell"] {{ font-weight: bold !important; text-transform: uppercase; }} </style>""", unsafe_allow_html=True)
...
if loading_error: # Lógica de erro como antes
        ...
        for corner_prefix, series_data in [("Azul", blue_s), ("Vermelho", red_s)]:
            ...
            if pd.notna(fighter_name_fc) and fighter_name_fc != "N/A":
                for task in all_tasks:
                    emoji_status = DEFAULT_EMOJI # Default se ID não encontrado
    ...
    column_config_editor = {
        ...
        "Lutador Azul (ID)": st.column_config.TextColumn("Lutador (A)", width="large", disabled=True),
        "Divisão": st.column_config.TextColumn(width="medium", disabled=True),
        "Lutador Vermelho (ID)": st.column_config.TextColumn("Lutador (V)", width="large", disabled=True),
    ...
    column_order_list.append("Lutador Vermelho (ID)"); column_order_list.append("Foto Vermelho") # Adicionado para garantir que estão na ordem

    # Legenda construída com emojis
    legend_parts = [f"{STATUS_TO_EMOJI.get(status_text, DEFAULT_EMOJI)}: {status_text}" for status_text in ["Done", "Requested", "---", "Pendente"]]
    help_text_general_legend_disp = ", ".join(legend_parts)

    for task_name_col in all_tasks: # Colunas de tarefa agora são TextColumn
    ...
    if not df_dashboard.empty:
        ...
        unique_fighters_in_event = set()
        for _, row in df_dashboard.iterrows():
            if row["Lutador Azul (ID)"] != "N/A": unique_fighters_in_event.add(row["Lutador Azul (ID)"].split(" - ",1)[0].strip()) # Pega ID
            if row["Lutador Vermelho (ID)"] != "N/A": unique_fighters_in_event.add(row["Lutador Vermelho (ID)"].split(" - ",1)[0].strip()) # Pega ID
        total_atletas_unicos_ev = len(unique_fighters_in_event)

        # Para estatísticas de tarefas, precisamos contar os emojis ou reverter para status textuais
        done_count = 0; req_count = 0; not_sol_count = 0; pend_count = 0; total_task_slots = 0
        for task in all_tasks:
            for corner in ["Azul", "Vermelho"]:
                col_name = f"{task} ({corner})"
                if col_name in df_dashboard.columns:
                    valid_fighter_mask = df_dashboard[f"Lutador {corner} (ID)"] != "N/A"
                    task_emojis_series = df_dashboard.loc[valid_fighter_mask, col_name]
                    total_task_slots += len(task_emojis_series)
                    done_count += (task_emojis_series == STATUS_TO_EMOJI["Done"]).sum()
                    req_count += (task_emojis_series == STATUS_TO_EMOJI["Requested"]).sum()
                    not_sol_count += (task_emojis_series == STATUS_TO_EMOJI["---"]).sum()
                    pend_count += (task_emojis_series == STATUS_TO_EMOJI["Pendente"]).sum()
        
        stat_cols = st.columns(5)
        stat_cols[0].metric("Lutas", total_lutas_evento)
        stat_cols[1].metric("Atletas Únicos", total_atletas_unicos_ev)
        stat_cols[2].metric(f"Tarefas {STATUS_TO_EMOJI['Done']} (Done)", done_count, help=f"De {total_task_slots} slots de tarefa.")
        stat_cols[3].metric(f"Tarefas {STATUS_TO_EMOJI['Requested']} (Requested)", req_count)
