...
def get_dashboard_style(font_size_px: int, num_tasks: int, fighter_width_pc: int, division_width_pc: int, division_font_size_px: int) -> str:
    ...
    <style>
        ...
        /* Restore header visibility for sidebar toggle */
...
if "dash_config_loaded" not in st.session_state:
    with st.spinner("Loading Dashboard Config..."):
        ...
        if "table_font_size" in loaded_conf:
            try: st.session_state.table_font_size = int(loaded_conf["table_font_size"])
            except: pass
        if "fighter_width" in loaded_conf:
            try: st.session_state.fighter_width = int(loaded_conf["fighter_width"])
            except: pass
        if "division_width" in loaded_conf:
            try: st.session_state.division_width = int(loaded_conf["division_width"])
            except: pass
        if "division_font_size" in loaded_conf:
            try: st.session_state.division_font_size = int(loaded_conf["division_font_size"])
            except: pass
...
# Event Selector
# Initialize key in session state if not present (from config load above or default)
...
st.session_state.table_font_size = st.sidebar.slider("Tamanho Geral da Fonte (px)", 10, 30, st.session_state.table_font_size, 1, on_change=save_current_config)
disable_sliders = len(selected_tasks) == 0
st.session_state.fighter_width = st.sidebar.slider("Largura Nome do Lutador (%)", 10, 40, st.session_state.fighter_width, 1, disabled=disable_sliders, on_change=save_current_config)
st.session_state.division_width = st.sidebar.slider("Largura Info da Luta (%)", 5, 25, st.session_state.division_width, 1, disabled=disable_sliders, on_change=save_current_config)
st.session_state.division_font_size = st.sidebar.slider("Fonte Info da Luta (px)", 10, 30, st.session_state.division_font_size, 1, on_change=save_current_config)
