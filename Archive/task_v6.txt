...
def get_latest_status(athlete_id, task, attendance_df):
    """Gets the latest status for a given athlete and task."""
    ...
    if "Timestamp" in athlete_records.columns:
        ...
        latest_record = athlete_records.sort_values(by="TS_dt", ascending=False).iloc[0]
    else:
...
# --- ATUALIZAÇÃO: Estado padrão modificado ---
default_ss = {
    ...
    "selected_status": "Requested", "selected_event": "Todos os Eventos", "fighter_search_query": ""
}
...
with st.container(border=True):
    ...
    with col_input_ps:
        st.session_state['user_id_input'] = st.text_input(
            ...
            max_chars=50, key="uid_w", label_visibility="collapsed", placeholder="Digite os 4 dígitos do seu PS"
        )
...
if st.session_state.user_confirmed and st.session_state.current_user_name!="User":
    ...
    # --- ATUALIZAÇÃO: Layout dos Filtros ---
    st.session_state.selected_task = st.selectbox(
        "Selecione a Tarefa:",
        tasks_for_select,
        index=tasks_for_select.index(st.session_state.selected_task) if st.session_state.selected_task in tasks_for_select else 0,
        key="tsel_w"
    )
    
    sel_task_actual = st.session_state.selected_task if st.session_state.selected_task != NO_TASK_SELECTED_LABEL else None
    
    # --- ATUALIZAÇÃO: Lógica para pré-calcular status e estatísticas ---
    if sel_task_actual and not df_athletes.empty:
        ...
        pending_count = status_counts.get('Pending', 0) + status_counts.get('---', 0) + status_counts.get('Not Registred', 0)
        requested_count = status_counts.get('Requested', 0)
        done_count = status_counts.get('Done', 0)
        
        # --- NOVO: Exibição das estatísticas ---
        st.markdown("##### Estatísticas da Tarefa")
        stat_cols = st.columns(3)
        with stat_cols[0]:
            st.metric("Pendente", f"{pending_count}")
        with stat_cols[1]:
            st.metric("Solicitado (Requested)", f"{requested_count}")
        with stat_cols[2]:
            st.metric("Concluído (Done)", f"{done_count}")
        st.divider()

    # --- ATUALIZAÇÃO: Filtro de status com botões de rádio ---
    status_options = ["Requested", "Done", "Pending"]
    ...
    if df_athletes.empty:
        ...
        if search_term:
            df_filtered = df_filtered[df_filtered["NAME"].str.lower().str.contains(search_term, na=False) | df_filtered["ID"].astype(str).str.lower().str.contains(search_term, na=False)]
        ...
        # --- Loop para Exibição dos Cards ---
        if not sel_task_actual:
            st.info("Selecione uma tarefa para ver as opções.", icon="ℹ️")

        for i_l, row in df_filtered.iterrows():
            ath_id_d, ath_name_d, ath_event_d = str(row["ID"]), str(row["NAME"]), str(row["EVENT"])
            curr_ath_task_stat_btn = row.get('current_task_status', 'Pending')

            # --- ATUALIZAÇÃO: Layout em duas colunas (Card e Botões) ---
            col_card, col_buttons = st.columns([2.5, 1])

            with col_card:
                # Monta HTML do Link do WhatsApp
                ...
                if mob_r:
                    ...
                    if phone_digits:
                        wa_link_html = f"""<p style='margin: 5px 0 10px 0; font-size:14px; text-align:center;'>
                                          <a href='https://wa.me/{html.escape(phone_digits, True)}' target='_blank' style='color:#25D366; text-decoration:none; font-weight:bold;'>
                                           WhatsApp
                                          </a></p>"""

                # Monta HTML da Tabela de Dados Pessoais
                pd_tbl_h = ""
                if st.session_state.show_personal_data:
                    ...
                    pd_tbl_h = f"""<div style='margin-top: 15px; border-top: 1px solid #444; padding-top: 15px;'>
                ...
                card_bg_col = "#2d3436" # Cor padrão
                if curr_ath_task_stat_btn == "Done": card_bg_col = "#143d14"
                elif curr_ath_task_stat_btn == "Requested": card_bg_col = "#B08D00"
                
                # Renderiza o Card
                st.markdown(f"""
                <div style='background-color:{card_bg_col}; padding:20px; border-radius:10px; box-shadow:2px 2px 5px rgba(0,0,0,0.3);'>
                    <div style='display:flex; align-items:center; gap:15px;'>
                        <img src='{html.escape(row.get("IMAGE","https://via.placeholder.com/80?text=No+Image")if pd.notna(row.get("IMAGE"))and row.get("IMAGE")else"https://via.placeholder.com/80?text=No+Image",True)}' style='width:80px; height:80px; border-radius:50%; object-fit:cover; border:2px solid white;'>
                        <div>
                            <h4 style='margin:0; text-align:center; font-size:1.5em;'>{html.escape(ath_name_d)}</h4>
                            <p style='margin:0; font-size:14px; color:#cccccc; text-align:center;'>{html.escape(ath_event_d)} (ID: {html.escape(ath_id_d)})</p>
                            {wa_link_html}
                        </div>
                    </div>
                    {pd_tbl_h}
            ...
            with col_buttons:
                if sel_task_actual:
                    ...
                    if curr_ath_task_stat_btn == "Requested":
                        if st.button(f"CONCLUIR", key=f"done_b_{ath_id_d}_{i_l}", type="primary", use_container_width=True, help=f"Marcar '{sel_task_actual}' como Concluído"):
                            registrar_log(ath_id_d, ath_name_d, ath_event_d, sel_task_actual, "Done", "", uid_l)
                            st.rerun()
                        if st.button(f"PENDENTE", key=f"pend_b_{ath_id_d}_{i_l}", type="secondary", use_container_width=True, help=f"Marcar '{sel_task_actual}' como Pendente"):
                            registrar_log(ath_id_d, ath_name_d, ath_event_d, sel_task_actual, "Pending", "", uid_l)
                            st.rerun()
                    else: # Se for "Pending", "Done" ou outro
                        btn_label = f"SOLICITAR"
                        if curr_ath_task_stat_btn == "Done":
                           btn_label = f"SOLICITAR NOVAMENTE"
                        
                        if st.button(btn_label, key=f"req_b_{ath_id_d}_{i_l}", type="primary", use_container_width=True, help=f"Solicitar '{sel_task_actual}'"):
