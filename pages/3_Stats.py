...
STATS_TAB_NAME = "Stats" # NOVA ABA PARA ESTATÍSTICAS
...
if st.session_state.user_confirmed and st.session_state.current_user_name!="Usuário": # Main App Content
    ...
    if df_athletes.empty: st.info("Nenhum atleta para exibir.")
        ...
        for i_l,row in df_filtered.iterrows():
            ...
            # --- Seção de Ações e Tarefas Específicas ---
            if sel_task_actual:
                if sel_task_actual=="Walkout Music":
                    # (Lógica do Walkout Music continua a mesma)
                ...
                # ### INÍCIO DA NOVA SEÇÃO DE ESTATÍSTICA ###
                elif sel_task_actual == "Estatística":
                    st.markdown("##### Estatísticas do Atleta")

                    # 1. Encontrar o registro mais recente para este atleta
                    latest_stats = None
                    has_previous_stats = False
                    if not df_stats.empty and 'athlete_id' in df_stats.columns:
                        ...
                        if not athlete_stats_df.empty:
                            ...
                            # Converte timestamp para datetime para ordenar corretamente
                    ...
                    # 2. Definir os campos e inicializar o estado da sessão
                    stat_fields = [
                    ...
                    # Popula os valores no estado da sessão com os dados mais recentes ou com strings vazias
                    ...
                    # 4. Layout dos campos e botões
                    col_b1, col_b2 = st.columns([0.7, 0.3])
                    
                    with col_b2:
                        # Botão para ligar/desligar modo de edição
                    ...
                    # Exibe os campos de input
                    c1, c2, c3 = st.columns(3)
                    cols = [c1, c2, c3]
                    
                    field_labels = {
                        'weight_kg': "Peso (kg)", 'height_cm': "Altura (cm)", 'reach_cm': "Envergadura (cm)",
                        'fight_style': "Estilo de Luta", 'country_of_representation': "País (Representação)", 'residence_city': "Cidade de Residência",
                        'team_name': "Nome da Equipe", 'tshirt_size': "Camiseta (Atleta)", 'tshirt_size_c1': "Camiseta (C1)",
                        'tshirt_size_c2': "Camiseta (C2)", 'tshirt_size_c3': "Camiseta (C3)"
                    }
                    ...
                    for i, field in enumerate(stat_fields):
                        with cols[i % 3]:
                            ...
                            st.text_input(
                                ...
                                help=f"Dado mais recente: {str(latest_stats[field]) if latest_stats is not None and field in latest_stats else 'N/A'}"
                            )

                    # 5. Botão de salvar (só aparece no modo de edição)
                    if is_editing:
                        if st.button(f"Salvar Alterações para {ath_name_d}", key=f"save_stats_{ath_id_d}", type="primary", use_container_width=True):
                            # Montar o dicionário de dados para salvar
                            new_data = {
                            ...
                            # Chamar a função para registrar na planilha
                            if add_stats_record(new_data):
                                st.session_state[edit_mode_key] = False # Sai do modo de edição
                                # Limpa chaves de estado para forçar recarga dos valores da planilha
                                for field in stat_fields:
                ...
                else: # Lógica para outras tarefas genéricas
