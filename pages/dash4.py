...
STATUS_INFO = {
    ...
    "Pending": {"class": "status-pending", "text": "Pending"}, # Mapeamento para o status de fallback
    # Mapeamentos do português para o inglês para consistência
...
# --- NEW: Function to calculate summary statistics ---
def calculate_task_summary(df_processed, task_list):
    """Calculates the count of each status for every task."""
    summary = {}
    for task in task_list:
        summary[task] = {"Done": 0, "Requested": 0, "Pending": 0, "---": 0}
        
        # Aggregate counts from both Blue and Red corners
        for corner in ["Azul", "Vermelho"]:
            col_name = f"{task} ({corner})"
            if col_name in df_processed.columns:
                # Extract the 'text' of the status from the dictionary in each cell
                status_texts = df_processed[col_name].apply(lambda x: x.get('text', 'Pending'))
                counts = status_texts.value_counts()
                for status, count in counts.items():
                    if status in summary[task]:
                        summary[task][status] += count
    return summary
...
# UPDATED: CSS to make .status-neutral transparent
st.markdown("""
<style>
    ...
    .status-neutral { background-color: transparent; } /* This makes the cell blend in */
...
st_autorefresh(interval=60000, key="dash_auto_refresh_v7")
...
for order, group in df_fc_disp.sort_values(by=[FC_EVENT_COL, FC_ORDER_COL]).groupby([FC_EVENT_COL, FC_ORDER_COL]):
    ...
    for prefix, series in [("Azul", bl_s), ("Vermelho", rd_s)]:
        if isinstance(series, pd.Series) and not series.empty:
            ...
            row_d[f"Foto {prefix}"], row_d[f"Lutador {prefix}"] = (pic if isinstance(pic, str) and pic.startswith("http") else ""), (f"{id} - {name}" if name != "N/A" else "N/A")
...
if dash_data_list:
    ...
    # Define max columns per row for responsiveness
    MAX_COLS = 6 
    cols = st.columns(MAX_COLS)
    col_index = 0
    
    for task, data in task_summary.items():
        # Filter out statuses with zero counts for a cleaner chart
        chart_data = {status: count for status, count in data.items() if count > 0 and status != "---"}
        if chart_data:
