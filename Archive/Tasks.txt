...
if st.session_state.user_confirmed:
    ...
    # --- ATUALIZAÇÃO: Nova seção de filtros com Multiselect e Pesquisa ---
    ...
    with filter_cols[1]:
        selected_events = st.multiselect("2. Filtrar por Evento(s)", options=unique_events, placeholder="Deixe em branco para todos os eventos")
    ...
    if st.session_state.selected_athletes:
        st.info(f"{len(st.session_state.selected_athletes)} atletas selecionados.")
    else:
        st.info("Nenhum atleta selecionado. Marque as caixas na primeira coluna para realizar ações em lote.")
