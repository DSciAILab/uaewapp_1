...
def registrar_log(ath_id: str, ath_name: str, ath_event: str, task: str, status: str, notes: str, user_log_id: str,
    ...
    try:
        ...
        # Simplificado para o exemplo, a sua lógica de número de linha pode ser mantida
        ...
        # Limpa todos os caches de dados relevantes para forçar o recarregamento
...
with st.container(border=True):
    # ... [código de autenticação do usuário - sem alterações] ...
    st.subheader("User")
...
if st.session_state.user_confirmed and st.session_state.current_user_name!="User":
    ...
    if sel_task_actual and not df_athletes.empty:
        ...
        chart_data = pd.DataFrame([
            {"Status": "Done", "Count": done_count}, {"Status": "Requested", "Count": requested_count}, {"Status": "Pending", "Count": pending_count}
        ])
        color_scale = alt.Scale(domain=['Done', 'Requested', 'Pending'], range=['#1E8449', '#F39C12', '#C0392B'])
    ...
    if df_athletes.empty:
        ...
        for i_l, row in df_filtered.iterrows():
            ...
            with col_card:
                # --- ATUALIZAÇÃO 1: Lógica de cor de fundo do card restaurada ---
                card_bg_col = "#2E2E2E" # Padrão
                if curr_ath_task_stat == "Done": card_bg_col = "#143d14" # Verde
                elif curr_ath_task_stat == "Requested": card_bg_col = "#B08D00" # Amarelo

                # Monta o card...
                # (O código para montar wa_link_html e pd_tbl_h permanece o mesmo)
                ...
                if mob_r:
                    ...
                    if phone_digits:
                        wa_link_html = f"""<p style='margin-top: 8px; font-size:14px; text-align:left;'><a href='https://wa.me/{html.escape(phone_digits, True)}' target='_blank' style='color:#FFF; text-decoration:none; font-weight:bold;'> WhatsApp</a></p>"""
                
                pd_content = f"<div style='flex-grow: 1; text-align: center; font-style: italic; color: #ccc;'>Dados pessoais ocultos.</div>"
                if st.session_state.show_personal_data:
                    ...
                    pd_content = f"""<div style='flex-grow: 1;'><table style='font-size:14px;color:white;border-collapse:collapse;width:100%;'>
                ...
                st.markdown(f"""
                <div style='background-color:{card_bg_col}; padding: 20px; border-radius: 10px;'>
                    <div style='display:flex; align-items:center; gap:20px;'>
                        ...
                        <div style='flex-grow: 1;'>
                            <h4 style='margin:0; font-size:1.6em; line-height: 1.2;'>{html.escape(ath_name_d)}</h4>
                            <p style='margin:0; font-size:1em; color:#cccccc;'>{html.escape(ath_event_d)} (ID: {html.escape(ath_id_d)})</p>
                            <p style='margin:5px 0 0 0; font-size:1em;'>Status: <strong>{curr_ath_task_stat}</strong></p>
                        </div>
                ...
                # --- ATUALIZAÇÃO 2: Lógica de cores dos badges restaurada ---
                badges_html = "<div style='display: flex; flex-wrap: wrap; gap: 8px; margin-top: 15px;'>"
                status_color_map = {"Done": "#28a745", "Requested": "#ffc107", "---": "#6c757d", "Pending": "#dc3545"}
