...
def load_stats_data(sheet_name: str = MAIN_SHEET_NAME, stats_tab_name: str = STATS_TAB_NAME):
    try:
        ...
        if df_stats.empty:
            return pd.DataFrame(columns=['athlete_id'])
...
def add_stats_record(data: dict, sheet_name: str = MAIN_SHEET_NAME, stats_tab_name: str = STATS_TAB_NAME):
    try:
        ...
        headers = ['timestamp', 'user', 'operation', 'athlete_id', 'weight_kg', 'height_cm', 'reach_cm', 'fight_style', 'country_of_representation', 'residence_city', 'team_name', 'tshirt_size', 'tshirt_size_c1', 'tshirt_size_c2', 'tshirt_size_c3']
...
if st.session_state.user_confirmed and st.session_state.current_user_name!="Usuário":
    ...
    if df_athletes.empty: st.info("Nenhum atleta para exibir.")
        ...
        if search_term: df_filtered_initial = df_filtered_initial[df_filtered_initial["NAME"].str.lower().str.contains(search_term, na=False) | df_filtered_initial["ID"].astype(str).str.lower().str.contains(search_term, na=False)]
        
        df_to_display = df_filtered_initial.copy()
        
        # ### CORREÇÃO PRINCIPAL APLICADA AQUI ###
        # Aplica o filtro de status apenas se houver status selecionados E a tarefa não for "Estatística"
        ...
        for i_l,row in df_to_display.iterrows():
            ...
            status_color_map = {"Requested": "#D35400", "Done": "#1E8449", "---": "#34495E"}; default_color = "#C0392B"
            ...
            if sel_task_actual: 
                if sel_task_actual=="Walkout Music":
                    ...
                    if st.button(f"Registrar Músicas para {ath_name_d}",key=f"reg_m_b_{ath_id_d}_{i_l}",type="primary",use_container_width=True):
                        ...
                        if any_m_reg: st.rerun() # st.rerun() é útil aqui para limpar os campos após o registro
                        else: st.warning("Nenhum link de música válido.",icon="⚠️")
                
                elif sel_task_actual == "Estatística":
                    st.markdown("##### Estatísticas do Atleta")
                    latest_stats = None; has_previous_stats = False
                    if not df_stats.empty and 'athlete_id' in df_stats.columns:
                        athlete_stats_df = df_stats[df_stats['athlete_id'] == ath_id_d].copy()
                        if not athlete_stats_df.empty:
                            has_previous_stats = True
                            athlete_stats_df['timestamp_dt'] = pd.to_datetime(athlete_stats_df['timestamp'], format='%d/%m/%Y %H:%M:%S', errors='coerce')
                            latest_stats = athlete_stats_df.sort_values(by='timestamp_dt', ascending=False).iloc[0]

                    stat_fields = ['weight_kg', 'height_cm', 'reach_cm', 'fight_style', 'country_of_representation', 'residence_city', 'team_name', 'tshirt_size', 'tshirt_size_c1', 'tshirt_size_c2', 'tshirt_size_c3']
                    for field in stat_fields:
                        key = f"stat_{field}_{ath_id_d}"
                        if key not in st.session_state:
                            st.session_state[key] = str(latest_stats[field]) if latest_stats is not None and field in latest_stats else ""
                    ...
                    for i, field in enumerate(stat_fields):
                        with cols[i % 3]:
                            st.text_input(field_labels.get(field, field), key=f"stat_{field}_{ath_id_d}", disabled=not is_editing, help=f"Dado mais recente: {str(latest_stats[field]) if latest_stats is not None and field in latest_stats else 'N/A'}")

                    if is_editing:
                        if st.button(f"Salvar Alterações para {ath_name_d}", key=f"save_stats_{ath_id_d}", type="primary", use_container_width=True):
                            new_data = {'timestamp': datetime.now().strftime("%d/%m/%Y %H:%M:%S"), 'user': st.session_state.get('current_user_name', 'System'), 'operation': "updated" if has_previous_stats else "created", 'athlete_id': ath_id_d}
                            for field in stat_fields: new_data[field] = st.session_state[f"stat_{field}_{ath_id_d}"]
                            if add_stats_record(new_data):
                                st.session_state[edit_mode_key] = False
                                for field in stat_fields:
                                    if f"stat_{field}_{ath_id_d}" in st.session_state:
                                        del st.session_state[f"stat_{field}_{ath_id_d}"]
                                # REMOVIDO st.rerun()
                    ...
                    if curr_ath_task_stat_btn=="Requested":
                        ...
                        with col_btn_act1:
                            if st.button(f"CONCLUIR '{sel_task_actual}' (Done)",key=f"mark_done_b_{ath_id_d}_{sel_task_actual.replace(' ','_')}_{i_l}",type="primary",use_container_width=True):
                                uid_l=st.session_state.get("current_user_ps_id_internal",st.session_state.current_user_id);registrar_log(ath_id_d,ath_name_d,ath_event_d,sel_task_actual,"Done","",uid_l)
                                # REMOVIDO st.rerun()
                        with col_btn_act2:
                            if st.button(f"CANCELAR SOL. '{sel_task_actual}' (---)",key=f"mark_pending_b_{ath_id_d}_{sel_task_actual.replace(' ','_')}_{i_l}",type="secondary",use_container_width=True):
                                uid_l=st.session_state.get("current_user_ps_id_internal",st.session_state.current_user_id);registrar_log(ath_id_d,ath_name_d,ath_event_d,sel_task_actual,"---","",uid_l)
                                # REMOVIDO st.rerun()
                    else:
                        ...
                        if st.button(btn_lbl_task,key=f"mark_stat_b_{ath_id_d}_{sel_task_actual.replace(' ','_')}_{i_l}",type=btn_type_task,use_container_width=True):
                            uid_l=st.session_state.get("current_user_ps_id_internal",st.session_state.current_user_id);registrar_log(ath_id_d,ath_name_d,ath_event_d,sel_task_actual,next_stat_log,"",uid_l)
                            # REMOVIDO st.rerun()
