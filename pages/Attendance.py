...
# Garante que as chaves existam desde o início para evitar erros.
if 'name_font_size' not in st.session_state:
    st.session_state.name_font_size = 18
if 'number_font_size' not in st.session_state:
    st.session_state.number_font_size = 48
if 'task_locked' not in st.session_state:
...
with st.sidebar:
    ...
    st.session_state.name_font_size = st.slider(
        "Tamanho do Nome do Atleta (px)", min_value=12, max_value=32, value=st.session_state.name_font_size)
    st.session_state.number_font_size = st.slider(
        "Tamanho do Número da Chamada (px)", min_value=24, max_value=96, value=st.session_state.number_font_size)

# --- CSS Dinâmico ---
# Gera o CSS com base nos valores dos sliders do session_state.
st.markdown(f"""
<style>
    /* Esconde o menu e cabeçalho padrão do Streamlit para maximizar espaço */
    div[data-testid="stToolbar"], #MainMenu, header {{
        visibility: hidden;
    }}
    /* Estilo para a foto circular do atleta */
    .athlete-photo {{
        width: 60px;
        height: 60px;
        border-radius: 50%;
    ...
    /* Estilo para a foto na coluna 'Finalizados' */
    .finished-photo {{
        width: 40px;
        height: 40px;
        border-radius: 50%;
    ...
    /* Classes de fonte controladas pelos sliders */
    .athlete-name {{
        font-size: {st.session_state.name_font_size}px !important;
        font-weight: bold;
        line-height: 1.2;
    }}
    .call-number {{
        font-size: {st.session_state.number_font_size}px !important;
        font-weight: bold;
        text-align: center;
        color: #17a2b8;
    }}
...
# --- [MODIFICADO] Lógica para ocultar a caixa de tarefa ---
if not st.session_state.task_locked:
    ...
    st.text_input(
        ...
        key="task_name_input",
        on_change=lock_task # Trava a tarefa quando o usuário pressiona Enter
    )
else:
    col_task_info, col_task_button = st.columns([4, 1])
    with col_task_info:
        st.success(f"Fila ativa para a tarefa: **{st.session_state.task_name_input}**")
    with col_task_button:
        st.button("Mudar Tarefa", on_click=unlock_task, use_container_width=True)

# O resto do aplicativo só roda se uma tarefa estiver definida e travada
if st.session_state.task_locked and st.session_state.task_name_input:
    ...
    if task_name not in st.session_state.get('tasks', {}):
        initialize_task_state(task_name, all_athletes_df)
    ...
    with col2:
        ...
        # --- [MODIFICADO] Lógica de destaque para o próximo ---
        for index, (athlete_id, athlete) in enumerate(checked_in_list):
            is_next = index == 0
            container_border_color = "#00BFFF" if is_next else "#808495" # Cor do destaque
            with st.container(border=True):
                num_col, pic_col, name_col = st.columns([1, 1, 2])
                with num_col:
                    st.markdown(f"<p class='call-number' style='color:{container_border_color};'>{athlete['checkin_number']}</p>", unsafe_allow_html=True)
                with pic_col:
                    st.markdown(f'<img class="athlete-photo" style="border-color:{container_border_color};" src="{athlete["pic"]}">', unsafe_allow_html=True)
                with name_col:
                    st.markdown(f"<p class='athlete-name'>{athlete['name']}</p>", unsafe_allow_html=True)
                    if is_next:
                        st.markdown("⭐ **PRÓXIMO!**")
    ...
    st.info("Digite um nome de tarefa e pressione Enter para começar.")
