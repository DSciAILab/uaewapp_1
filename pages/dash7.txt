...
# --- ATUALIZAÇÃO: Lógica de contagem corrigida ---
def calculate_task_summary(df_processed, task_list):
    """Calculates the count of each status for every task based on the precise definition."""
    summary = {}
    for task in task_list:
        ...
        for corner in ["Azul", "Vermelho"]:
            ...
            if col_name in df_processed.columns:
                # Extrai o texto do status (ex: "Done", "Requested", "Pending", "---")
                status_texts = df_processed[col_name].apply(lambda x: x.get('text', 'Pending'))
                counts = status_texts.value_counts()
                
                for status, count in counts.items():
                    # Contagem explícita baseada na definição correta
                    if status == "Done":
                        summary[task]["Done"] += count
                    elif status == "Requested":
                        summary[task]["Requested"] += count
                    elif status in ["Pending", "Not Registered"]:
                        # Apenas os status que indicam ausência de registro contam como Pendente
                        summary[task]["Pending"] += count
                    # Status como "---" ou "Not Requested" são intencionalmente ignorados
    return summary
...
st_autorefresh(interval=60000, key="dash_auto_refresh_v10")
