...
def get_all_athletes_status(df_athletes: pd.DataFrame, df_attendance: pd.DataFrame, fixed_task: str) -> pd.DataFrame:
    ...
    if df_attendance is None or df_attendance.empty:
        base['current_task_status'] = Config.STATUS_BASE
    ...
    # Filtrar registros relevantes na presença (com aliases)
    ...
    # Fallback determinístico quando TS_dt é NaT (linhas mais recentes por posição)
    df_task["__idx__"] = np.arange(len(df_task))

    # Merge entre atletas e presença normalizada
    merged = pd.merge(
...
def last_task_other_event_by_name(
    ...
    Args:
        ...
        fixed_task (str): Tarefa fixa (ex: "Photoshoot").
        aliases (list[str]): Aliases/variações da tarefa.
...
def render_athlete_card(row: pd.Series, last_info: tuple, badges_html: str) -> str:
    ...
    Args:
        ...
        last_info (tuple): (data, evento) do último registro da tarefa fixa em outro evento.
    ...
    task_status_html = f"<small style='color:#ccc;'>{task_label}: <b>{html.escape(curr_ath_task_stat)}</b></small>"
    arrival_status_html = (
...
def registrar_log(
    ...
    try:
        ...
        # >>> Alinha com as colunas esperadas: ["#", "Event", "Name", "Fighter", "Task", "Status", "User", "Timestamp", "TimeStamp", "Notes"]
...
with st.spinner("Carregando dados..."):
    ...
    # Garante lista de strings em tasks_raw
...
for i_l, row in df_filtered.iterrows():
    ...
    if tasks_raw:
        ...
        for task_name in tasks_raw:
            # Pula a tarefa fixa desta página para evitar redundância
            ...
            if not df_attendance.empty:
                ...
                if not task_records.empty:
                    # Escolhe o registro mais recente com fallback determinístico
...
Este aplicativo Streamlit foi desenvolvido para gerenciar o status de Photoshoot dos atletas.
...
3. Pré-processamento de dados para normalização e conversão de timestamps (priorizando 'TimeStamp' e caindo para 'Timestamp').
...
7. Botões para atualizar o status da tarefa fixa (Request, Done, Cancel).
8. Exibição de badges para outras tarefas e histórico do último registro da tarefa fixa em outro evento.

OTIMIZAÇÕES APLICADAS:
1. `@st.cache_data` com TTLs adequados para reduzir recargas.
2. Processamento em massa do status por merge + ordenação determinística (fallback de índice) quando TS_dt é ausente.
3. Labels dinâmicos com `Config.FIXED_TASK` (evitando textos hardcoded).
4. Helper genérico `make_task_mask` para aliases da tarefa (ex.: "photoshoot", "photo shoot", "photo").
5. Append alinhado às colunas ["#", "Event", "Name", "Fighter", "Task", "Status", "User", "Timestamp", "TimeStamp", "Notes"].

ESTRUTURA:
- Constantes e configurações: `class Config`.
- Funções auxiliares: normalização, parse de datas, aliases de tarefa.
- Carregamento de dados: com cache e validações.
- Processamento de dados: `preprocess_attendance`, `get_all_athletes_status`, `last_task_other_event_by_name`.
- Renderização de UI: `render_athlete_card`.
- Registro de logs: `registrar_log`.
- Lógica principal: filtros, ordenação, sumário e cards de atletas.

O código está modular, performático e pronto para manutenção.
"""
